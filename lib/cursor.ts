...
import { normalizeDegree } from "./geo/geo_angle";
import Design from "./index";
import { isEqual } from "./utils/loadsh";
...
export default class Cursor {
  ...
  setCursor(cursor: ICursor) {
    this.normalizeCursor(cursor)
    if (isEqual(cursor, this.cursor)) {
      return;
    }

    this.cursor = cursor;
    const clsPrefix = 'design-cursor-';
