...
export default class Canvas {
  ...
  Cursor: Cursor
  constructor(private target: string | HTMLDivElement, private design: Design) {
    this.width = 0
    this.height = 0
  ...
  //初始化视口
  initView() {
    const target = (typeof this.target  === 'string' ?  document.querySelector(this.target)  : this.target) as HTMLDivElement
